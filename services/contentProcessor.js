...
const { YtTranscript } = require('yt-transcript'); // Import the fallback library
...
const processContent = async (contentId) => {
    ...
    try {
        ...
        if (url.includes('youtube.com') || url.includes('youtu.be')) {
            ...
            try {
                // ** Primary Method: youtube-transcript **
                ...
                if (!extractedText || extractedText.length < 20) {
                   throw new Error('Transcript is empty or too short.'); // This will trigger the fallback
                }
                console.log('Transcript found successfully with primary method.');

            } catch (primaryError) {
                console.warn(`Primary method failed: ${primaryError.message}. Trying fallback 'yt-transcript'...`);

                // ** Fallback Method: yt-transcript **
                try {
                    const videoId = getYouTubeVideoId(url);
                    if (!videoId) throw new Error("Could not extract YouTube video ID from URL.");
                    
                    const transcriptFetcher = new YtTranscript({ videoId });
                    const transcript = await transcriptFetcher.getTranscript();

                    if (!transcript || transcript.length === 0) {
                        throw new Error('Fallback method also found no transcript.');
                    }
                    extractedText = transcript.map((line) => line.text).join(' ').trim();
                    console.log('Transcript found successfully with fallback method.');

                } catch (fallbackError) {
                    console.error(`Fallback method also failed: ${fallbackError.message}`);
                    throw new Error("This video does not have a transcript available from any provider.");
                }
            }
        } else {
            // Article Workflow remains the same
        ...
        // --- Language Detection, Summarization, and Indexing logic remains the same ---
